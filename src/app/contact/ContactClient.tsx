      ...
      >
        ...
        <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
          <motion.div variants={STAGGER} initial="hidden" animate="show" className="max-w-[48ch]">
            ...
            <motion.p variants={FADE_UP}
              className="text-[1.02rem] leading-[1.85] text-[#7A7A7A]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_480px] gap-16 lg:gap-20 items-start">
            ...
            >
              ...
              <motion.ul role="list" variants={FADE_UP} className="flex flex-col gap-7 mb-14">
                {DETAILS.map(({ label, value, href, icon }) => (
                  <li key={label} className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-xl bg-[#5F7A61]/10 text-[#5F7A61] flex items-center justify-center shrink-0">
                      {icon}
                    </div>
                    <div>
                      <p className="text-[0.7rem] uppercase tracking-[0.16em] font-semibold text-[#7A7A7A]/60 mb-1">
                        {label}
                      </p>
                      {href ? (
                        <a
                          href={href}
                          className="text-[0.95rem] font-medium text-[#1F2A44] hover:text-[#5F7A61] transition-colors duration-200 focus-visible:outline-none focus-visible:underline"
                        >
                          {value}
                        </a>
                      ) : (
                        <p className="text-[0.95rem] font-medium text-[#1F2A44] whitespace-pre-line leading-relaxed">
                          {value}
                        </p>
                      )}
                    </div>
              ...
              <motion.div variants={FADE_UP}>
                <p className="text-[0.7rem] uppercase tracking-[0.16em] font-semibold text-[#7A7A7A]/60 mb-4">
                  Find us
                </p>
                <div
                  role="img"
                  aria-label="Map showing Nispruh Yog Centre location — interactive map coming soon"
                  className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden border border-[#d6c9b8]/60 bg-gradient-to-br from-[#e8e4dc] via-[#dcd7ce] to-[#d0cbbf] flex flex-col items-center justify-center gap-3"
                >
                  ...
                  >
                    {[0,1,2,3,4].map((i) => (
                      <line key={`h${i}`} x1="0" y1={i * 75} x2="400" y2={i * 75} stroke="#1F2A44" strokeWidth="1" />
                    ))}
                    {[0,1,2,3,4,5].map((i) => (
                      <line key={`v${i}`} x1={i * 80} y1="0" x2={i * 80} y2="300" stroke="#1F2A44" strokeWidth="1" />
                    ))}
                    {/* Suggested roads */}
                    <path d="M0 140 Q100 120 200 150 Q300 180 400 155" stroke="#1F2A44" strokeWidth="2" fill="none" />
                    <path d="M160 0 Q170 100 165 150 Q158 220 162 300" stroke="#1F2A44" strokeWidth="2" fill="none" />
                  </svg>

                  {/* Pin */}
                  <div className="relative z-10 flex flex-col items-center gap-2">
                    <div className="w-10 h-10 rounded-full bg-[#5F7A61] flex items-center justify-center shadow-[0_4px_16px_rgba(95,122,97,0.4)]">
                    ...
                    <span className="text-[0.75rem] font-medium text-[#1F2A44]/70 bg-white/80 backdrop-blur-sm px-3 py-1 rounded-full">
                      Nispruh Yog Centre
                    </span>
                  </div>
                  <p className="relative z-10 text-[0.68rem] uppercase tracking-[0.18em] text-[#7A7A7A]/50 font-medium">
                    Map placeholder
                  </p>
                </div>
              </motion.div>
              ...
              >
                {[
                  ...
                  <li key={item} className="flex items-center gap-3 text-[0.85rem] text-[#7A7A7A]">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#5F7A61]/60 shrink-0" aria-hidden="true" />
                    {item}
                  </li>
                ))}
              </motion.ul>
            ...
            <motion.div
              ...
              viewport={{ once: true, margin: "-60px" }}
              className="rounded-2xl border border-[#d6c9b8]/60 bg-white p-8 sm:p-10 shadow-[0_8px_48px_rgba(31,42,68,0.06)]"
            >
              <p className="text-[0.72rem] uppercase tracking-[0.2em] text-[#5F7A61] font-semibold mb-2">
                Send a message
              </p>
              <h2 className="text-[1.2rem] font-semibold text-[#1F2A44] mb-7">
                We&apos;d love to hear from you
              </h2>

              <ContactForm />

              <p className="mt-5 text-center text-[0.78rem] text-[#7A7A7A]/60">
      ...
      <section className="py-20 bg-[#1F2A44] border-t border-white/[0.06]">
        <div className="mx-auto max-w-3xl px-5 sm:px-8 text-center">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <span className="block text-[3rem] text-[#5F7A61]/30 font-serif leading-none mb-5 select-none" aria-hidden="true">"</span>
